        ...
        LabyHelp.getInstace().getExecutor().submit(new Runnable() {
            ...
            public void run() {
                ...
                for (Map.Entry<String, Integer> uuidStringEntry : list) {
                    labyPlayer.sendDefaultMessage(EnumChatFormatting.YELLOW + "" + i + EnumChatFormatting.WHITE + ": " + EnumChatFormatting.YELLOW + EnumChatFormatting.BOLD + UUIDFetcher.getName(UUID.fromString(uuidStringEntry.getKey())).toUpperCase() + EnumChatFormatting.WHITE + " with " + EnumChatFormatting.YELLOW + EnumChatFormatting.BOLD + uuidStringEntry.getValue() + EnumChatFormatting.WHITE + LabyHelp.getInstace().getTranslationManager().getTranslation("invite.points"));
                    i++;
                }
            }
        });
